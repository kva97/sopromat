...
class Balk{
    ...
    distributed_load(coords_array, name = 1, direction = 'up'){
        ...
        this.draw_text(coords_array[1] + x1 + 5, coords_array[0] + x1, y1 - 40, name, 'black', 8);
        this.draw_text(coords_array[1] + x1 - 5, coords_array[0] + x1, y1 - 45, 'q', 'black', 12);
    }
    concentrated_force(x, name = 1, direction = 'up'){
        this.draw_text(x, x + 27, y1 - 40, name, 'black', 8);
        this.draw_text(x, x + 17, y1 - 45, 'F', 'black', 12);
        if(direction == 'up'){
    ...
    moment(x, name = 1, direction = 'left'){
        let R = 20;
        ...
        if(direction == 'right'){
            ...
            this.draw_text(x, x + 55, y1 - 40, name, 'black', 8);
            this.draw_text(x, x + 40, y1 - 45, 'M', 'black', 12);
        }
        if(direction == 'left'){
            this.draw_line(x - R * Math.cos(Math.PI + Math.PI / 6), y1 - 40 - R * Math.sin(- Math.PI / 6), x, y1);
            this.draw_arrow(x + R * Math.cos(Math.PI + Math.PI / 3), y1 - 37, x + R * Math.cos(Math.PI - Math.PI / 6), y1 - 40 - R * Math.sin(- Math.PI / 6));
            this.draw_text(x, x - 40, y1 - 40, name, 'black', 8);
            this.draw_text(x, x - 55, y1 - 45, 'M', 'black', 12);
        }
    }
}
...
function init(){
    ...
    for(let i = 0; i < document.getElementsByClassName('force').length; i++){
        balk.concentrated_force(force.coords[i] + x1, i + 1, force.direction[i]);
    ...
    for(let i = 0; i < document.getElementsByClassName('load').length; i++){
        balk.distributed_load(load.coords[i], i + 1, load.direction[i]);
    ...
    for(let i = 0; i < document.getElementsByClassName('moment').length; i++){
        balk.moment(moment.coords[i] + x1, i + 1, moment.direction[i]);
...
function support_calc(x, relative_coord){
    ...
    for(coord of moment.coords){
        if(coord == x){
            support_value += get_moment(moment.values[moment.coords.indexOf(coord)], coord, relative_coord);
        }
    }
    if(!isNaN(support.values[0]) && !isNaN(support.values[1])){
        for(coord of support.coords){
            if(coord == x){
                support_value += get_force(support.values[support.coords.indexOf(coord)], coord, relative_coord);
            }
        }
    }
...
function max(arr) {
    ...
    while (len--) {
        if(Math.abs(arr[len]) > Max) Max = Math.abs(arr[len]);
    }
    if(arr[len] >= 0) return Max;
    else return -Max;
}
...
function force_diagram(){
    ...
    for(coord of x_coords){
        if(coord != x2 - x1 && coord != 0) coord += 1;
...
function moment_diagram(){
    ...
    for(let i = 0; i <= x2 - x1; i += 1){
        ...
        for(coord of all_coords){
            ...
            if(coord[1] === 'moment' && coord[0] <= relative && i > coord[0]){
                x_moment += get_moment(moment.values[moment.coords.indexOf(coord[0])], i, relative);
            }
            if(coord[2] === 'load' && i >= coord[0] && i <= coord[1]){
    ...
    for(coord of x_coords){
        if(coord != x2 - x1){
            text = (x_moments[~~coord] + ((x_moments[~~coord + 1] - x_moments[Math.round(coord)]) * (coord - ~~coord))).toFixed(2).toString();
        }
    ...
    if(Math.abs(max(x_moments)) > Math.abs(max(moment.values)) || moment.values.length == 0){
        ...
        for(coord of x_coords){
            if(x_moments.indexOf(Max) == Math.round(coord)) break;
            else if(x_moments.indexOf(Max) > Math.round(coord)) continue;
                ...
                balk.draw_text(x_moments.indexOf(Max) + x1, x_moments.indexOf(Max) + x1 + 8 * text.length, max_y + 10, text);
            }
        }
    }
...
function linear_displacements(){
    ...
    for(coord of x_coords){
        ...
        else if(lindisp.indexOf(Max) > Math.round(coord)) continue;
        else if(isFinite(Max)){
            text = Max.toFixed(2).toString();
